...
public class RechargeEnterpriseView extends AbstractExcelView {

    private static final Logger log = LoggerFactory.getLogger(RechargeEnterpriseView.class);

    private ReportService reportService;

    public RechargeEnterpriseView(ReportService reportService) {
    ...
    protected void buildExcelDocument(Map<String, Object> map, HSSFWorkbook workbook, HttpServletRequest request,
        ...
        if (log.isDebugEnabled()) {
            log.debug("building RechargeEnterpriseView...");
        }

        LocalDate d = (LocalDate) map.get("model");

        List<RechargeDetails> rechargeDetails = reportService.findRechargeEnterpriseByDate(d);
        ...
        int i = 1;
        for (RechargeDetails details : rechargeDetails) {
            ...
            setCellValue(sheet, style, i, 0, details.getAcctCustId(), row);
...
//            setCellValue(sheet, style, i, 5, "", row);
//            setCellValue(sheet, style, i, 6, "", row);
